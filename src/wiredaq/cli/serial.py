...
Same pipeline shape as the datagram slice (`wiredaq-slice`), but the link is a raw **byte
stream** instead of a datagram transport:
    ...
    synthetic node(s)
        ...
        → collector             (per-node seq tracking)        ← unchanged from the slice
        → metrics (+ optional CSV)

The Collector, sinks, codec, and synthetic node are byte-for-byte the same objects the
datagram slice uses. Only the transport and receiver changed — which is the whole point of
...
or without installing: `python -m wiredaq.cli.serial ...`.
"""
...
from wiredaq.daq_sim.transports.serial_transport import (
