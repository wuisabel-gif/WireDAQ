...
`tools/daq_sim` and the C firmware codec — must encode each input to exactly
...
- `reference_encoder.py` — the test oracle and generator. It is the canonical, simple
  implementation of `protocol/packet_schema.yaml`. It is **not** the production codec.
...
for v in json.load(open("protocol/golden/vectors.json"))["vectors"]:
...
```bash
python3 protocol/golden/reference_encoder.py
```
