...
``protocol/packet_schema.yaml``.

Unlike ``protocol/golden/reference_encoder.py`` (the deliberately-simple oracle that
*generates* the golden vectors), this is the codec the running simulator uses. Both must
reproduce ``protocol/golden/vectors.json`` byte-for-byte — that equivalence is enforced
